void new class TenantMigration{
    constructor()
    {
        this.initDatatable()
    }
    ...
    initDatatable = () => {
        this.dataTable = $('#tenant_master_list').KTDatatable({
            data:{
                type:'remote',
                source:{
                    read:{
                        url:`/tenant/migration/list`,
                        method:'GET'
                    }
                },
                pageSize: 10,
            ...
            columns:[
                {
                    field:'filename',
                    title:'Filename',
                    template: ({filename}) => `<span>${filename}</span>`

                },
                {
                    field:'lastModified',
                    title:'Date Created',
                    template: ({lastModified}) =>  `<span>${humanDate(lastModified)}</span>`
                ...
                },
                {
                    ...
                    template: function({path}) {
                        return `
                        <a href="/migrate/${path}" class="btn btn-sm btn-clean btn-icon mr-2" title="Migrate">
